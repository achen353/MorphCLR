...
class MoCo(nn.Module):
    ...
    def __init__(
        ...
        self.encoder_q = base_encoder(
            num_classes=dim, pretrained=args.imagenet_pretrained
        )
    ...
    def _dequeue_and_enqueue(self, keys, q_selector=None, sample_idx=None):
        ...
        assert (
            self.K % batch_size == 0
        ), f"self.K={self.K}, batch_size={batch_size}"  # for simplicity
        ...
        if q_selector is not None:
            for iQ in range(self.qMult):
                ...
                self.queue_idx[iQ, local_ptr : local_ptr + batch_size] = sample_idx[
                    q_selector == iQ
                ]
        else:
    ...
    def forward(
        ...
        if isinstance(q, tuple) or isinstance(
            q, list
        ):  # handle the case features are returned too
            extra_outputs["q_fm"] = nn.functional.normalize(q[2], dim=1)
            extra_outputs["q"] = nn.functional.normalize(
                torch.mean(torch.mean(q[2], dim=3), dim=2), dim=1
            )
            q = q[0]
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            if isinstance(k, tuple) or isinstance(
                k, list
            ):  # handle the case features are returned too
                k = k[0]
            ...
            # undo shuffle
            if not self.debug:
                k = self._batch_unshuffle_ddp(
                    k, idx_unshuffle
                )  # no need to unshuffle the "isedge_k" as they are no longer used beyond this point
        ...
        if self.args.multi_q:
            for iQ in range(self.qMult):
                ...
                ixx = q_selector == iQ
                _l_neg = torch.einsum(
                    "nc,ck->nk", [q[ixx], self.queue[iQ][:, :qlim].clone().detach()]
                )
                if sample_idx is not None:
            ...
            _l_neg = torch.einsum(
                "nc,ck->nk", [q, self.queue[:, :qlim].clone().detach()]
            )
            if sample_idx is not None:
        ...
        self._dequeue_and_enqueue(
            k, q_selector if self.args.multi_q else None, sample_idx
        )
...
def concat_all_gather_interlace(tensor):
    ...
    *** Warning ***: torch.distributed.all_gather has no gradient.
    ...
    output = torch.cat(tensors_gather, dim=0)
    inds = [
        torch.arange(i, len(g) * len(tensors_gather), len(tensors_gather))
        for i, g in enumerate(tensors_gather)
    ]
...
def load_model(args, return_epoch=False):
    ...
    model = models_wrappers.__dict__[args.arch](
        num_classes=args.moco_dim, pretrained=args.imagenet_pretrained
    )
