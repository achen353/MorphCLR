...
def demo(query_domains, search_domains, data_root):
    def show_res(query_domain, search_domain, num_results_per_domain=5):
        ...
        if search_domain == "all":
            for d_name, file_path in search_domains.items():
                with open(file_path, "rb") as fp:
                    ...
                    src_nn_fit = NearestNeighbors(
                        n_neighbors=num_results_per_domain, algorithm="auto", n_jobs=-1
                    ).fit(src_data[1])
        ...
        def query(dst_idx):
            ...
            for s_domain, s_data in src_data_dict.items():
                _, top_n_matches_ids = s_data[1].kneighbors(
                    dst_data[1][dst_idx : dst_idx + 1]
                )
                top_n_labels = s_data[0][2][top_n_matches_ids][0]
                src_img_pths = [
                    os.path.join(data_root, s_data[0][0][ix])
                    for ix in top_n_matches_ids[0]
                ]
                ...
                for p in src_img_pths:
                    src_cl = p.split("/")[-2]
                    src_cl = (
                        f'<span style="color: #ff0000"> {src_cl} </span>'
                        if src_cl == q_cl
                        else src_cl
                    )
                    src_file = p.split("/")[-1]
    ...
    interactive = interact_manual(
        show_res,
        query_domain=list(query_domains.keys()),
