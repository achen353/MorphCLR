...
parser.add_argument(
    "--data", metavar="DIR", help="comma_separated list of paths to textfiles"
)
parser.add_argument(
    "-a",
    "--arch",
...
parser.add_argument(
    "--save_n_epochs", default=10, type=int, help="Save every N epochs (default 10)"
)
...
parser.add_argument(
    ...
    default="hed",
    choices=["hed", "canny"],
    help="type of edges to use: canny, hed",
)  # Default should be trainable hed
# params for hed
parser.add_argument(
    "--static_hed", action="store_false", help="Do not train hed", dest="train_hed"
)
parser.add_argument(
    "--hed_loss_type",
    default="l2_hed",
    choices=["l2_hed", "l2_canny", "l1_canny"],
    help="l2_hed (default), l2_canny, l1_canny",
)
parser.add_argument(
    "--hed_loss_w", default=1.0, type=float, help="weight for hed training loss"
)
parser.add_argument(
    "--canny_blur_rad", default=5, type=int, help="weight for hed training loss"
)
parser.add_argument(
    "--stretch_edge", action="store_true", help="stretch edge map to [0,1]"
)
parser.add_argument(
    "--edges_sigma", default="1.0", type=str, help="canny sigma (default: 1.0)"
)
# params for canny
parser.add_argument(
    "--edges_dil",
...
parser.add_argument("--no_cc", action="store_true", help="if on will not do CenterCrop")

parser.add_argument(
    "--img2_is_sketch", action="store_true", help="if on will not do CenterCrop"
)
parser.add_argument(
    "--skeleton",
    type=int,
...
def setup(args):
    ...
    work_folder = os.path.join(
        args.exp_root, args.exp_folder_name.format(**args.__dict__)
    )
    ...
    if (
        args.is_root
    ):  # TODO: fix logger so the outputs are in stdout, and maybe have several file for each process
        ...
        try:
            if os.path.isfile(log_file_path) and args.is_root:
                ...
                bckp_pth = log_file_path.replace(
                    f"log_{args.rank}.out", f"log_{args.rank}_{timestamp_str}.out"
                )
